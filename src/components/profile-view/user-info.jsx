        ...
        <div className="p-2"> 
            ...
            <ListGroup variant="flush" className="custom-profile-list rounded border shadow-sm">
                ...
                {(user.firstName || user.lastName) && (
                    <ListGroup.Item><strong>Full Name:</strong> {`${user.firstName || ""} ${user.lastName || ""}`.trim()}</ListGroup.Item>
                )}
                {user.birthDate && (
                    <ListGroup.Item><strong>Birth Date: </strong> {new Date(user.birthDate).toLocaleDateString(undefined, {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                        timeZone: 'UTC'
